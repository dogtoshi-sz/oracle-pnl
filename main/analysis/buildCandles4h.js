...
function get4hBucket(ts) {
  return Math.floor(ts / FOUR_HOUR_MS) * FOUR_HOUR_MS;
}
...
function build4hCandlesForToken(tokenAddress) {
  ...
  for (const s of snapshots) {
    const bucketTs = get4hBucket(s.timestamp);
